    ...
    public void ShouldSetTheCorrespondingFieldsForAProcessedShowcase()
    {
        ...
        Showcase showcase = new ShowcaseBuilder()
            ...
            .FeaturedItems(new List<SubItem>
                {
                    new("slug","title", "icon", "teaser", "link", "contentType", "image-url.jpg", string.Empty, "body text", new List<SubItem>(), string.Empty, string.Empty, EColourScheme.Teal, string.Empty, string.Empty, string.Empty, string.Empty, string.Empty)
                })
            .Build();
