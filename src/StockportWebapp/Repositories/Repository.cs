...
namespace StockportWebapp.Repositories
{
    public class Repository : IRepository
    {
        ...
        private Dictionary<string, string> authenticationHeaders;
        ...
        public async Task<HttpResponse> Put<T>(HttpContent content, string slug = "")
        {
            ...
            return await _httpClient.PutAsync(url, content, authenticationHeaders);
        }

        public async Task<HttpResponse> Delete<T>(string slug = "")
        {
            var url = $"{_urlGenerator.UrlFor<T>(slug)}";
            return await _httpClient.DeleteAsync(url, authenticationHeaders);
        }
        ...
        {
            var url = $"{_urlGenerator.UrlFor<Group>(slug)}/administrators/{email}";
            return await _httpClient.PutAsync(url, user, authenticationHeaders);
        }

        public async Task<HttpResponse> AddAdministrator(HttpContent user, string slug, string email)
        {
            var url = $"{_urlGenerator.UrlFor<Group>(slug)}/administrators/{email}";
            return await _httpClient.PostAsync(url, user, authenticationHeaders);
        }
